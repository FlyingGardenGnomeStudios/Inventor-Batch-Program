...
Public Class Main
                ...
                If Strings.Right(item.FullFileName, Len(item.FullFileName) - InStrRev(item.FullFileName, "\")) = Trim(LVSubFiles.CheckedItems(x).Text) Then
                    ...
                    If Checkin = True Then
                        Try
                            item.Save()
                            item.Close(True)
                            writeDebug("Drawing " & item.DisplayName & " closed with save")
                            Exit For
                    ...
                    Else
                        item.Close(True)
                        writeDebug("Drawing " & item.DisplayName & " closed without save")
                        Exit For
                    End If
                End If
            Next
    ...
    Public Sub CloseLater(Name As String, oDoc As Document)
        ...
        If CloseLater = True Then
            ...
            Catch
                writeDebug("Failed to close " & Name & ". Perhaps not yet saved.")
            End Try
